...
$oportunidades = $conexion->prepare("SELECT opu.id as 'opu_id',opu.ingresos_estimados,opu.tema,are.tipoarea_id ,opu.fecha_cierre,
tare.descripcion as 'area'  ,cue.id as 'cue_id' ,cue.nombreempresa,con.id as 'con_id', con.nombre,
con.apellido,con.email,con.telefono
...
WHERE  testo.id=2 order by opu.ingresos_estimados DESC");
$oportunidades->execute();
$roportunidades = $oportunidades->execute();

$roportunidades = $oportunidades->fetchAll(PDO::FETCH_OBJ);

require '../views/oportunidades/oportunidades_abiertas.views.php';
